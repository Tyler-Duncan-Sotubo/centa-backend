...
const schema_2 = require("../../drizzle/schema/schema");
...
let VerificationService = class VerificationService {
    ...
    async generateVerificationToken(userId) {
        ...
        const user = await this.db.select().from(schema_2.users).where((0, drizzle_orm_1.eq)(schema_2.users.id, userId));
        if (user.length === 0) {
    ...
    async verifyToken(dto) {
        ...
        await this.db
            ...
            .where((0, drizzle_orm_1.eq)(schema_2.users.id, existingToken[0].user_id))
            .execute();
        await this.db
            .update(schema_1.verificationToken)
            .set({ is_used: true })
