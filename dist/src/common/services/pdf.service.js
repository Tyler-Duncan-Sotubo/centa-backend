...
let PdfService = class PdfService {
    ...
    formatMoneyForPdf(amount) {
        return (0, formatCurrency_1.formatCurrency)(amount, 'NGN', 'en-NG', {
    ...
    async generatePayslipPdf(payslipId) {
        ...
        const payrollMonthLabel = new Date(payslip.payroll_month + '-01').toLocaleString('en-US', {
            month: 'long',
            year: 'numeric',
        });
        const money = (n) => this.formatMoneyForPdf(n);
        const htmlContent = `
      <!DOCTYPE html>
      <html>
        <head>
