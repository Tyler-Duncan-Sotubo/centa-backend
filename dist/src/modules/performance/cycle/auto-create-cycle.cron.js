            ...
            try {
                ...
                if (!latestCycle) {
                    ...
                    try {
                        ...
                        this.logger.warn(`Skip existing '${name}' {for ${company.name}} + ${e.message}`);
                    }
                    continue;
                }
                ...
                this.logger.error(`Auto cycle failed for ${company?.name ?? company?.id}`, err?.stack ?? String(err));
            }
        }
    }
    ...
    generateNextCycleRange(frequency, fromDate, generateCurrent = true) {
        ...
        switch (frequency) {
            case 'monthly': {
                ...
                name = (0, date_fns_1.format)(start, 'MMM-yyyy').toUpperCase();
                break;
            }
            case 'quarterly': {
