...
let GoalsService = class GoalsService {
    constructor(db, auditService, cache) {
        ...
        this.cache = cache;
        this.ttlListSeconds = 5 * 60;
        this.ttlItemSeconds = 5 * 60;
    }
    tags(companyId) {
    ...
    async create(dto, user) {
        ...
        if (ownerIds.length) {
            ...
            await this.db.insert(performance_goals_schema_1.performanceGoals).values(goalInstances).execute();
        }
    ...
    async findAll(companyId, status) {
        return this.cache.getOrSetVersioned(companyId, ['goals', 'list', status || 'all'], async () => {
            ...
            const updates = await this.db
                ...
                .where((0, drizzle_orm_1.inArray)(performance_goal_updates_schema_1.performanceGoalUpdates.goalId, goalIds))
    ...
    async findAllByEmployeeId(companyId, employeeId, status) {
        return this.cache.getOrSetVersioned(companyId, ['goals', 'by-employee', employeeId, status || 'all'], async () => {
            const today = new Date().toISOString().slice(0, 10);
            const conditions = [
                (0, drizzle_orm_1.eq)(performance_goals_schema_1.performanceGoals.companyId, companyId),
    ...
    async findOne(id, companyId) {
        return this.cache.getOrSetVersioned(companyId, ['goals', 'one', id], async () => {
