"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ReservedDaysController = void 0;
const common_1 = require("@nestjs/common");
const reserved_days_service_1 = require("./reserved-days.service");
const create_reserved_day_dto_1 = require("./dto/create-reserved-day.dto");
const update_reserved_day_dto_1 = require("./dto/update-reserved-day.dto");
const base_controller_1 = require("../../../common/interceptor/base.controller");
const current_user_decorator_1 = require("../../auth/decorator/current-user.decorator");
const jwt_auth_guard_1 = require("../../auth/guards/jwt-auth.guard");
let ReservedDaysController = class ReservedDaysController extends base_controller_1.BaseController {
    constructor(reservedDaysService) {
        super();
        this.reservedDaysService = reservedDaysService;
    }
    create(createReservedDayDto, user) {
        return this.reservedDaysService.create(createReservedDayDto, user);
    }
    findAll(user) {
        return this.reservedDaysService.findAll(user.companyId);
    }
    findOne(id) {
        return this.reservedDaysService.findOne(id);
    }
    findByEmployee(id) {
        return this.reservedDaysService.findByEmployee(id);
    }
    update(id, updateReservedDayDto, user) {
        return this.reservedDaysService.update(id, updateReservedDayDto, user);
    }
    remove(id) {
        return this.reservedDaysService.remove(id);
    }
};
exports.ReservedDaysController = ReservedDaysController;
__decorate([
    (0, common_1.Post)(),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
    (0, common_1.SetMetadata)('permissions', ['reserved_days.manage']),
    __param(0, (0, common_1.Body)()),
    __param(1, (0, current_user_decorator_1.CurrentUser)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [create_reserved_day_dto_1.CreateReservedDayDto, Object]),
    __metadata("design:returntype", void 0)
], ReservedDaysController.prototype, "create", null);
__decorate([
    (0, common_1.Get)(),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
    (0, common_1.SetMetadata)('permissions', ['reserved_days.read']),
    __param(0, (0, current_user_decorator_1.CurrentUser)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", void 0)
], ReservedDaysController.prototype, "findAll", null);
__decorate([
    (0, common_1.Get)(':id'),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
    (0, common_1.SetMetadata)('permissions', ['reserved_days.read']),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
], ReservedDaysController.prototype, "findOne", null);
__decorate([
    (0, common_1.Get)('employee/:id'),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
    (0, common_1.SetMetadata)('permissions', ['reserved_days.manage']),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
], ReservedDaysController.prototype, "findByEmployee", null);
__decorate([
    (0, common_1.Patch)(':id'),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
    (0, common_1.SetMetadata)('permissions', ['reserved_days.manage']),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Body)()),
    __param(2, (0, current_user_decorator_1.CurrentUser)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, update_reserved_day_dto_1.UpdateReservedDayDto, Object]),
    __metadata("design:returntype", void 0)
], ReservedDaysController.prototype, "update", null);
__decorate([
    (0, common_1.Delete)(':id'),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
    (0, common_1.SetMetadata)('permissions', ['reserved_days.manage']),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
], ReservedDaysController.prototype, "remove", null);
exports.ReservedDaysController = ReservedDaysController = __decorate([
    (0, common_1.Controller)('reserved-days'),
    __metadata("design:paramtypes", [reserved_days_service_1.ReservedDaysService])
], ReservedDaysController);
//# sourceMappingURL=reserved-days.controller.js.map