...
let AssessmentsService = AssessmentsService_1 = class AssessmentsService {
    ...
    companyVersionKey(companyId) {
        return `pa:v:${companyId}`;
    }
    userListVersionKey(userId) {
        return `pa:ulistv:${userId}`;
    }
    teamVersionKey(managerId, cycleId) {
        return `pa:teamv:${managerId}:${cycleId}`;
    }
    summaryVersionKey(revieweeId, cycleId) {
        return `pa:summaryv:${revieweeId}:${cycleId}`;
    }
    attendanceVersionKey(userId, companyId) {
        return `pa:attv:${userId}:${companyId}`;
    }
    async getV(key) {
    ...
    async dashboardKey(companyId, filters) {
        const v = await this.getV(this.companyVersionKey(companyId));
        const f = JSON.stringify(filters ?? {});
        return `pa:v${v}:${companyId}:dashboard:${f}`;
    }
    async userListKey(userId) {
        const v = await this.getV(this.userListVersionKey(userId));
        return `pa:ulistv${v}:user:${userId}:list`;
    }
    async teamKey(managerId, cycleId) {
        const v = await this.getV(this.teamVersionKey(managerId, cycleId));
        return `pa:teamv${v}:team:${managerId}:cycle:${cycleId}`;
    }
    async reviewSummaryKey(revieweeId, cycleId) {
        const v = await this.getV(this.summaryVersionKey(revieweeId, cycleId));
        return `pa:summaryv${v}:summary:${revieweeId}:${cycleId}`;
    }
    async attendanceKey(userId, companyId, start, end) {
        const v = await this.getV(this.attendanceVersionKey(userId, companyId));
        return `pa:attv${v}:att:${userId}:${companyId}:${start}:${end}`;
    }
    async burst(opts) {
        const jobs = [];
        ...
        if (opts.reviewerId)
            jobs.push(this.bumpV(this.userListVersionKey(opts.reviewerId)));
        if (opts.revieweeId)
            jobs.push(this.bumpV(this.userListVersionKey(opts.revieweeId)));
        if (opts.managerId && opts.cycleId)
            jobs.push(this.bumpV(this.teamVersionKey(opts.managerId, opts.cycleId)));
        if (opts.revieweeId && opts.cycleId)
            jobs.push(this.bumpV(this.summaryVersionKey(opts.revieweeId, opts.cycleId)));
        if (opts.attendance) {
            const { userId, companyId, start, end } = opts.attendance;
            jobs.push(this.bumpV(this.attendanceVersionKey(userId, companyId)));
            if (start && end)
                jobs.push(this.cache.del(await this.attendanceKey(userId, companyId, start, end)));
        }
    ...
    async getAssessmentsForDashboard(companyId, filters) {
        ...
        return this.cache.getOrSetCache(key, async () => {
            ...
            if (filters?.departmentId && filters.departmentId !== 'all') {
                whereClauses.push((0, drizzle_orm_1.eq)(schema_1.employees.departmentId, filters.departmentId));
            }
    ...
    async getAssessmentsForUser(userId) {
        const key = await this.userListKey(userId);
    ...
    async getTeamAssessments(managerId, cycleId) {
        const key = await this.teamKey(managerId, cycleId);
    ...
    async getReviewSummary(revieweeId, cycleId) {
        const key = await this.reviewSummaryKey(revieweeId, cycleId);
    ...
    async getAttendanceSummaryForCycle(userId, companyId, cycle) {
        const key = await this.attendanceKey(userId, companyId, cycle.startDate, cycle.endDate);
