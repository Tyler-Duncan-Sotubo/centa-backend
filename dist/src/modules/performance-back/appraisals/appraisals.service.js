...
let AppraisalsService = AppraisalsService_1 = class AppraisalsService {
    constructor(db, auditService, companySettingsService, logger, cache) {
        this.db = db;
        this.auditService = auditService;
        this.companySettingsService = companySettingsService;
        this.logger = logger;
        this.cache = cache;
        this.logger.setContext(AppraisalsService_1.name);
    }
    oneKey(id) {
        return `appraisals:${id}:detail`;
    }
    listKey(companyId, cycleId) {
        return `appraisals:${companyId}:cycle:${cycleId}:list`;
    }
    dashboardKey(companyId, employeeId) {
    ...
    async burst(opts) {
        ...
        if (opts.companyId && opts.employeeId)
            jobs.push(this.cache.del(this.dashboardKey(opts.companyId, opts.employeeId)));
    ...
    async create(createDto, companyId, userId) {
        ...
        const [employee] = await this.db
            ...
            .execute();
        if (!employee) {
            this.logger.warn({ employeeId: createDto.employeeId }, 'appraisals:create:no-employee');
        ...
        if (!managerId) {
            ...
            if (!defaultManager) {
                this.logger.warn({ employeeId: createDto.employeeId }, 'appraisals:create:no-manager-default');
        ...
        if (existing.length > 0) {
            this.logger.warn({ employeeId: createDto.employeeId, cycleId: createDto.cycleId }, 'appraisals:create:duplicate');
    ...
    async findAll(companyId, cycleId) {
        ...
        return this.cache.getOrSetCache(key, async () => {
            ...
            const rows = await this.db
                ...
                .execute();
            this.logger.debug({ companyId, cycleId, count: rows.length }, 'appraisals:list:db:done');
            return rows;
        });
    }
    async findDashboardForEmployee(companyId, employeeId) {
        const key = this.dashboardKey(companyId, employeeId);
        this.logger.debug({ key, companyId, employeeId }, 'appraisals:dashboard:cache:get');
        return this.cache.getOrSetCache(key, async () => {
    ...
    async findOne(id, companyId) {
        ...
        const row = await this.cache.getOrSetCache(key, async () => {
            ...
            const [record] = await this.db
                ...
                companyId: performance_appraisals_schema_1.appraisals.companyId,
            })
                .from(performance_appraisals_schema_1.appraisals)
                .leftJoin(emp, (0, drizzle_orm_1.eq)(performance_appraisals_schema_1.appraisals.employeeId, emp.id))
                .leftJoin(mgr, (0, drizzle_orm_1.eq)(performance_appraisals_schema_1.appraisals.managerId, mgr.id))
        ...
        const { companyId: _c, employeeId: _e, ...safe } = row;
        return safe;
    }
    async updateManager(appraisalId, newManagerId, user) {
        const { id: userId, companyId } = user;
        this.logger.info({ appraisalId, newManagerId, userId }, 'appraisals:updateManager:start');
    ...
    async update(id, updateDto, user) {
        ...
        const current = await this.findOne(id, user.companyId);
        const [updated] = await this.db
            .update(performance_appraisals_schema_1.appraisals)
            .set(updateDto)
    ...
    async restartAppraisal(appraisalId, user) {
        this.logger.info({ appraisalId, userId: user.id }, 'appraisals:restart:start');
        const [existing] = await this.db
            .select({
            id: performance_appraisals_schema_1.appraisals.id,
            employeeId: performance_appraisals_schema_1.appraisals.employeeId,
...
exports.AppraisalsService = AppraisalsService = AppraisalsService_1 = __decorate([
