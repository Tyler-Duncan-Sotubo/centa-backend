...
let ShiftsService = class ShiftsService {
        ...
        for (const row of rows) {
            ...
            if (typeof rawDays === 'string') {
                try {
                    ...
                    throw new common_1.BadRequestException(`Invalid WorkingDays format for "${row['Name'] ?? row['name']}": must be a JSON array`);
                }
            }
            else {
                workingDays = rawDays;
            ...
            if (locationName) {
                ...
                if (!match) {
                    throw new common_1.BadRequestException(`Unknown location name "${locationName}" in row "${row['Name'] ?? row['name']}"`);
                }
                locationId = match;
            }
    ...
    async findAll(companyId) {
        return this.cache.getOrSetVersioned(companyId, ['attendance', 'shifts', 'list'], async () => {
            return this.db
                .select({
                id: shifts_schema_1.shifts.id,
                name: shifts_schema_1.shifts.name,
