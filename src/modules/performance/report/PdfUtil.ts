...
export class PdfUtil {
  ...
  static renderAppraisalReportHtml(data: any[]) {
    const rows = data
      .map(
        (r) => `
      <tr>
  ...
  static renderTopEmployeesHtml(data: any[]) {
    const rows = data
      .map(
        (r) => `
      <tr>
  ...
  static renderHeatmapHtml(heatmap: Record<string, Record<string, number>>) {
    const rows = Object.entries(heatmap)
      .flatMap(([competency, levels]) =>
  ...
  static renderGoalReportHtml(data: any[]) {
    const rows = data
      .map(
        (g) => `
      <tr>
  ...
  static renderFeedbackReportHtml(data: any[]) {
    const rows = data
      .flatMap((item) =>
        item.responses.map(
          (r) => `
  ...
  static renderAssessmentSummaryHtml(data: any[]) {
    const rows = data
      .map(
        (a) => `
      <tr>
