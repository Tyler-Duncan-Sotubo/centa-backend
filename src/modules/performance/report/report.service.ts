...
export class ReportService {
  ...
  // ---------- key helpers ----------
  private kReportFilters(companyId: string) {
    return `rpt:${companyId}:filters`;
  }
  private kAppraisal(companyId: string, f?: GetAppraisalReportDto) {
    return `rpt:${companyId}:appraisal:${this.ser(f)}`;
  }
  private kGoals(companyId: string, f?: GetGoalReportDto) {
    return `rpt:${companyId}:goals:${this.ser(f)}`;
  }
  private kFeedback(companyId: string, f: GetFeedbackReportDto) {
    return `rpt:${companyId}:feedback:${this.ser(f)}`;
  }
  private kAssessments(companyId: string, f?: GetAssessmentReportDto) {
    return `rpt:${companyId}:assess:${this.ser(f)}`;
  }
  private kTop(companyId: string, f: GetTopEmployeesDto) {
    return `rpt:${companyId}:top:${this.ser(f)}`;
  }
  private kHeatmap(companyId: string, cycleId?: string) {
    return `rpt:${companyId}:heatmap:${cycleId ?? 'active'}`;
  }
  private kParticipation(companyId: string, cycleId?: string) {
    return `rpt:${companyId}:participation:${cycleId ?? 'active'}`;
  }
  private kOverview(companyId: string) {
    return `rpt:${companyId}:overview`;
  }

  // stable serializer for keys
  private ser(obj: any) {
    if (!obj) return 'none';
  ...
  async getFeedbackReport(user: User, filters: GetFeedbackReportDto) {
    ...
    return this.cache.getOrSetCache(key, async () => {
      ...
      for (const r of responses) {
        if (!grouped[r.feedbackId]) grouped[r.feedbackId] = [];
      ...
      const result = await this.db
        .select({
          ...
          promotionRecommendation:
            assessmentConclusions.promotionRecommendation,
  ...
  async getTopEmployees(user: User, filter: GetTopEmployeesDto) {
    ...
    return this.cache.getOrSetCache(key, async () => {
      ...
      const rows = await this.db
        ...
        .innerJoin(
          employees,
          eq(performanceAssessments.revieweeId, employees.id),
        )
        .leftJoin(jobRoles, eq(jobRoles.id, employees.jobRoleId))
        .leftJoin(departments, eq(departments.id, employees.departmentId))
        .leftJoin(
  ...
  async getCompetencyHeatmap(user: User, filters?: { cycleId: string }) {
    ...
    return this.cache.getOrSetCache(key, async () => {
      ...
      for (const row of result) {
        if (!heatmap[row.competencyName]) heatmap[row.competencyName] = {};
        heatmap[row.competencyName][row.levelName ?? 'Unrated'] = row.count;
      }

      this.logger.debug(
        { groups: Object.keys(heatmap).length },
        'report:heatmap:db:done',
      );
      return heatmap;
    });
  }
  ...
  async getPerformanceOverview(user: User) {
    ...
    return this.cache.getOrSetCache(key, async () => {
      // 1) fetch active cycles (used for some metrics)
      ...
      if (!appraisalCycle && !performanceCycle) return [];

      // 2) execute subreports (these are cached themselves)
      const [
        appraisalsReport,
        goalsReport,
        peerFeedback,
      ...
      const completedAppraisals = appraisalsReport.filter(
      ...
      const avgTimeToCompleteDays =
        completedAppraisals && appraisalCycle
          ? appraisalsReport
      ...
      const recommendationCounts = appraisalsReport.reduce(
        (acc, a) => {
          const r = (a as any).promotionRecommendation || 'none';
          acc[r] = (acc[r] || 0) + 1;
          return acc;
        },
      ...
      const completedGoals = goalsReport.filter(
        (g) => g.status === 'completed',
      ).length;
      const overdueGoals = goalsReport.filter(
      ...
      const anonymityRate = totalFeedback
        ? (peerFeedback.filter((f: any) => f.isAnonymous).length +
            mgrFeedback.filter((f: any) => f.isAnonymous).length +
            selfFeedback.filter((f: any) => f.isAnonymous).length) /
          totalFeedback
        : 0;
